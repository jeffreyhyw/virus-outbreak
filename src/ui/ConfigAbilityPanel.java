...
public class ConfigAbilityPanel{
	...
	ArrayList<VirusAbility> att_list;
...
//        panel.setLayout(new GridLayout(1, 1));
		...
		System.out.println("ConfigAbilityPanel");
//        panel.add(filler);
	...
	void generateDate() {
		...
		att_list = game.getVirus().getAbilityList();
	}
	...
	void generateConfigurePanel(JPanel configurePanel) {
		...
		for(int i = 0; i < att_list.size(); i ++) {
      		    		...
      		    		for(int k = 0; k < att_panel.getComponentCount(); k ++) {
      		    			if(att_panel.getComponent(k).getName() == null) {
      		    				...
      		    				if(!att_list.get(pos).checkLevel()) {
      		    					System.out.println(Constants.max_lv);
      		    					break;
      		    				}
      		...
      		JButton infoBtn = new JButton(Constants.info);
